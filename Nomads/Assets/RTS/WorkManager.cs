...
namespace RTS
{
    public static class WorkManager
    {
        ...
        public static List<WorldObject> FindNearbyObjects(Vector3 position, float range, Dictionary<Transform, int> unitIDs)
        {
            ...
            {
                Transform objTransform = hitColliders[i].transform;
                if (objTransform)
                {
                    WorldObject obj = objTransform.GetComponent<WorldObject>();
                    if (obj && !nearbyObjectIds.Contains(obj.ObjectId))
                    {
                        nearbyObjectIds.Add(unitIDs[objTransform]);
                        nearbyObjects.Add(obj);
                    }
                    if (Vector3.Distance(objTransform.transform.position, position) > range)
                    {
                        nearbyObjectIds.Remove(unitIDs[objTransform]);
